'use server'

import { supabase } from "@/lib/supabase";
import { revalidatePath } from "next/cache";
import { registrarLog } from "@/lib/logs";

export async function obtenerClientes() {
    const {data, error} = await supabase.from('clientes').select("*")
    if(error) throw error;
    return data
}

export default async function crearNuevoCliente(data) {
    const {data: cliente, error} = await supabase
        .from("clientes")
        .insert([
            {
            nombre_completo: data.nombre,
            telefono: data.telefono,
            email: data.email,
            direccion: data.direccion,
            notas: data.notas
        },
    ])
    .select()
    .single();

    if(error) {
        throw error;
    }

    await registrarLog('Cliente creado', cliente.nombre_completo)
    revalidatePath('/dashboard/clientes');
    revalidatePath('/dashboard/clientes-inactivos');
    revalidatePath('/dashboard');
    return cliente;

}

export async function obtenerClientePorId(id) {
    const {data, error} = await supabase
    .from("clientes")
    .select("*")
    .eq("id", id)
    .single()

  if (error) {
    return null;
  }

  return data
} 

export async function editarCliente(id, data) {
  const { data: cliente, error } = await supabase
    .from("clientes")
    .update({
      nombre_completo: data.nombre,
      telefono: data.telefono,
      email: data.email,
      direccion: data.direccion,
      notas: data.notas,
    })
    .eq("id", id)
    .select()
    .single();

  if (error) throw error;

  await registrarLog('Cliente editado', cliente.nombre_completo)
  revalidatePath('/dashboard/clientes');
  revalidatePath(`/dashboard/clientes/${id}`);
  revalidatePath('/dashboard/clientes-inactivos');
  revalidatePath('/dashboard');
  return cliente;
}

export async function eliminarClientePorId(clienteId) {
  const { data: clienteExistente } = await supabase
    .from('clientes')
    .select('nombre_completo')
    .eq('id', clienteId)
    .maybeSingle()

  const { error: errorCitas } = await supabase
    .from('citas')
    .delete()
    .eq('cliente_id', clienteId)

  if (errorCitas) throw errorCitas

  const { error } = await supabase
    .from('clientes')
    .delete()
    .eq('id', clienteId)

  if (error) throw error

  await registrarLog('Cliente eliminado', clienteExistente?.nombre_completo || clienteId)
  revalidatePath('/dashboard/clientes');
  revalidatePath('/dashboard/calendario');
  revalidatePath('/dashboard/clientes-inactivos');
  revalidatePath('/dashboard');
}

export async function obtenerConteoClientesInactivos() {
    const inactivos = await obtenerClientesInactivos()
    return inactivos.length
}

export async function obtenerConteoClientesActivos() {
    const total = await obtenerTotalClientes()
    const inactivos = await obtenerConteoClientesInactivos()
    return total - inactivos
}
  
export async function obtenerTotalClientes() {
    const { count, error } = await supabase
        .from('clientes')
        .select('*', { count: 'exact', head: true })

    if (error) throw error
    return count || 0
}

export async function obtenerClientesNuevosEsteMes() {
    const hoyStr = new Date().toLocaleDateString('sv', { timeZone: 'America/El_Salvador' })
    const [year, month] = hoyStr.split('-').map(Number)
    const primerDia = new Date(year, month - 1, 1).toISOString()

    const { count, error } = await supabase
        .from('clientes')
        .select('*', { count: 'exact', head: true })
        .gte('created_at', primerDia)

    if (error) throw error
    return count || 0
}

export async function obtenerClientesInactivos() {
    // Clientes cuya última cita fue hace más de 30 días
    const hoy = new Date()
    const treintaDiasAtras = new Date(hoy)
    treintaDiasAtras.setDate(hoy.getDate() - 30)

    // Formato local YYYY-MM-DD para comparar con la DB
    const year = treintaDiasAtras.getFullYear()
    const month = String(treintaDiasAtras.getMonth() + 1).padStart(2, '0')
    const day = String(treintaDiasAtras.getDate()).padStart(2, '0')
    const fechaLimite = `${year}-${month}-${day}`

    // 1. Obtener todas las citas para determinar la última actividad de cada uno
    const { data: todasLasCitas, error: errCitas } = await supabase
        .from('citas')
        .select('cliente_id, fecha')
        .order('fecha', { ascending: false })
    
    if (errCitas) throw errCitas

    const ultimaCitaPorCliente = {}
    todasLasCitas.forEach(cita => {
        if (!ultimaCitaPorCliente[cita.cliente_id]) {
            ultimaCitaPorCliente[cita.cliente_id] = cita.fecha
        }
    })

    // 2. Obtener todos los clientes
    const { data: todosLosClientes, error: errClientes } = await supabase
        .from('clientes')
        .select('*')
    
    if (errClientes) throw errClientes

    // 3. Filtrar: Clientes cuya última cita fue hace más de 30 días
    return todosLosClientes
        .map(cliente => ({
            ...cliente,
            ultima_cita: ultimaCitaPorCliente[cliente.id] || null
        }))
        .filter(cliente => {
            // Incluir clientes que tienen última cita hace más de 30 días
            if (!cliente.ultima_cita) return false 
            return cliente.ultima_cita < fechaLimite
        })
        .sort((a, b) => b.ultima_cita.localeCompare(a.ultima_cita))
}


